...
def main():
    parser = argparse.ArgumentParser(description="Predict letter from a single image file.")
    parser.add_argument('input_image', type=str, help='Path to input image file')
    parser.add_argument('--model', type=str, default=str(Path('result') / 'handwriting_resnet18_best.pt'), 
                       help='Path to model checkpoint (.pt)')
    parser.add_argument('--labels', type=str, default=str(Path('result') / 'labels.json'), 
                       help='Path to labels JSON')
    parser.add_argument('--device', type=str, default='auto', choices=['auto', 'cpu', 'cuda'], 
                       help='Device selection')
    parser.add_argument('--top-k', type=int, default=5, help='Show top-K predictions')
    parser.add_argument('--show-image', action='store_true', help='Display the input image')
    ...
    pred_label, confidence, top_predictions = predict_letter(
        model, image, tfms, device, label_list, args.top_k
    )
